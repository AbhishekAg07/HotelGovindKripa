...
const FALLBACK_MENU_ITEMS = [
  ...
  },
  {
    ...
    name: "Royal Biryani",
    category: "nonveg",
    description: "Fragrant basmati rice layered with spices and slow-cooked meat.",
    price: 340
  }
];
...
async function loadMenuItems() {
  ...
  try {
    ...
    setMenuItems([]);
    status.textContent = "Menu is unavailable right now. Please call us for the latest dishes.";
  }
}
...
function getApiBase() {
  const isLocalAppServer = location.hostname === "127.0.0.1" && location.port === "8787";
  const isLocalhostAppServer = location.hostname === "localhost" && location.port === "8787";

  if (isLocalAppServer || isLocalhostAppServer) {
    return "";
  }
